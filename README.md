...
The 0.4.x line adds an opt-in dominant-clone sparse-admixture model. One BCR
...
Post-fit tree pruning, split-back, and refinement utilities are available from
the dedicated module rather than the top-level namespace:
...
ACT and ACT-initializer classes are not part of the 0.4.3 release.
...
```text
0.2.2 ───────────────► 0.4.0 ─► 0.4.1 ─► 0.4.3
   \
    └──► 0.3.0 ─► 0.3.1  [experimental, discontinued]
```

The 0.4.x lineage is based directly on 0.2.2. Version 0.4.3 is a release-hygiene
patch over that lineage and does not change the statistical model. The 0.3.x
artifacts remain available only for reproducibility.
